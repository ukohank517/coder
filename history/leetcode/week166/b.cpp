...
public:
    vector<vector<int> > groupThePeople(vector<int>& groupSizes) {
        ...
        rep(i, groupSizes.size()){
            number[groupSizes[i]].pb(i);
            
            if(number[groupSizes[i]].size() == groupSizes[i]) {
                ans.pb(number[groupSizes[i]]);
                number[groupSizes[i]].clear();
            }
        }
...
int main(){
    ...
    vector<vector<int> > ans = Solution().groupThePeople(a);
    rep(i, ans.size()){
        rep(j, ans[i].size()){
            cout << ans[i][j] << ",";
        }
